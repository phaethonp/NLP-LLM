...
<html>
<head>
    <title>Web Summarizer Service</title>
    ...
    <style>
        ...
        .price-tag { font-size: 1.2em; font-weight: bold; color: #32325d; margin-bottom: 10px; }
    </style>
</head>
<body>
    <h2>Web Summarizer Service</h2>
    <div id="alert" class="alert hidden"></div>
    
    <form id="payment-form">
        <div class="section">
            <h3>URL to Summarize</h3>
            <div class="form-row">
                <label for="url">Webpage URL</label>
                <input type="url" id="url" required 
                       placeholder="https://example.com/article">
                <div class="help-text">
                    Enter the URL of the webpage you want to summarize
                </div>
            </div>
            <div class="price-tag">
                Price: $5.00
        ...
        <button type="submit">
            ...
            <span class="button-text">Get Summary ($5.00)</span>
        </button>
    </form>
    ...
    <script>
        ...
        form.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                const response = await fetch('/process-summary', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        url: url,
...
def process_summary():
    try:
        data = request.json
        logger.info(f"Processing summary request for URL: {data['url']}")
        ...
        try:
            customer = stripe.Customer.create(
                ...
                description="Web summarizer customer",
                metadata={
                    'source': 'web_summarizer',
                    'url': data['url']
                }
            )
        ...
        try:
            ...
            if isinstance(result, dict) and 'body' in result:
                try:
                    ...
                    return jsonify({
                        "success": True,
                        "summary": body.get('summary', 'Summary not available'),
        ...
        error_message = str(e)
        logger.error(f"Summary processing error: {error_message}")
        return jsonify({
            "error": error_message,
...
if __name__ == '__main__':
    print("\n🌐 Starting Web Summarizer Service")
    print("Make sure the Lambda container is running with:")
    print("docker run -p 9000:8080 web-summarizer-lambda\n")
